...
void AdditionalLightsToon_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, half4 Shadowmask,
						...
						out float3 Diffuse, out float3 Specular, out bool IsInShadow)
{
    float3 diffuseColor = 0;
    float3 specularColor = 0;
...
// Soglia minima: se distanceAttenuation è troppo basso, il punto è fuori raggio luce
            if (distAtt > 0.001)
            {
                if (shadowAtt > 0.001)
                {
        // Illuminato dentro il raggio
                    diffuseColor += light.color * shadowAtt * ToonAttenuation(lightIndex, WorldPosition, PointLightBands, SpotLightBands);
                }
                else
                {
        // In ombra dentro il raggio
                    IsInShadow = true;
                    //diffuseColor += float3(0.1, 0.0, 0.2); // Viola scuro
                    diffuseColor += light.color * shadowAtt * ToonAttenuation(lightIndex, WorldPosition, PointLightBands, SpotLightBands);
                }
            }
        }
						...
						out float3 Diffuse, out float3 Specular, out bool IsInShadow)
{
	...
	PointLightBands, SpotLightBands, Diffuse, Specular, IsInShadow);
}
